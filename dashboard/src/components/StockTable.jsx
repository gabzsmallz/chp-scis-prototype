...
function StockCell({ value, threshold }) {
    ...
    if (qty < 0) return <TableCell>—</TableCell>TableCell>;
    ...
    if (!reports || reports.length === 0) {
          return (
                  <Paper sx={{ p: 3, textAlign: 'center' }}>
                          <Typography color="text.secondary">No stock reports received yet.</Typography>Typography>
                          <Typography variant="caption">CHPs need to submit stock reports via the CHT app.</Typography>Typography>
                  </Paper>Paper>
                );
    }
  
    // Flatten FHIR SupplyDelivery resources into table rows
                ...
                <Table size="small">
                        <TableHead sx={{ bgcolor: '#1a6e3c' }}>
                          ...
                          <TableCell key={col} sx={{ color: 'white', fontWeight: 'bold' }}>
                                          <TableSortLabel active={orderBy === col} direction={orderBy === col ? order : 'asc'}
                                                              onClick={() => toggleSort(col)} sx={{ color: 'white !important', '& .MuiTableSortLabel-icon': { color: 'white !important' } }}>
                                            {col.toUpperCase()}
                          ...
                          <TableCell key={c.key} align="center" sx={{ color: 'white', fontWeight: 'bold', fontSize: '0.75rem' }}>
                            {c.label}
                                              ...
                                              <TableCell align="center" sx={{ color: 'white', fontWeight: 'bold' }}>STATUS</TableCell>TableCell>
                                  </TableRow>TableRow>
                        </TableHead>TableHead>
                        <TableBody>
                          {sorted.map((row) => (
                        <TableRow key={row.id} hover sx={{ '&:nth-of-type(odd)': { bgcolor: '#f9f9f9' } }}>
                                      <TableCell>{row.chp}</TableCell>TableCell>
                                      <TableCell>{row.chu}</TableCell>TableCell>
