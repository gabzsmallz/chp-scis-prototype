...
function TabPanel({ children, value, index }) {
    return value === index ? <Box sx={{ py: 3 }}>{children}</Box>Box> : null;
}
...
export default function App() {
  ...
  useEffect(() => {
        fetchData();
        // Auto-refresh every 60 seconds during simulation session
        ...
        <Box sx={{ flexGrow: 1, minHeight: '100vh', bgcolor: '#f5f5f5' }}>
          {/* Header */}
              ...
              <Container maxWidth="xl" sx={{ mt: 3 }}>
                {/* Error banner */}
                {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>Alert>}
              
                {/* Stockout alert banner */}
                {!loading && alerts.length > 0 && (
                    <Alert severity="warning" sx={{ mb: 2 }}>
                                <strong>{alerts.length} CHP(s)</strong>strong> have reported stockouts in their latest submission.
                    </Alert>Alert>
                      )}
